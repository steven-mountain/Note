...
在使用vkCmdBindPipeline切换渲染管线后，如果您的渲染管线使用了VK_DYNAMIC_STATE_VIEWPORT和VK_DYNAMIC_STATE_SCISSOR，那么您需要使用vkCmdSetViewport和vkCmdSetScissor重新设置视口和裁剪矩形。这是因为，当您使用VK_DYNAMIC_STATE_VIEWPORT和VK_DYNAMIC_STATE_SCISSOR时，视口和裁剪矩形的状态是由vkCmdSetViewport和vkCmdSetScissor设置的。如果您的渲染管线没有使用VK_DYNAMIC_STATE_VIEWPORT和VK_DYNAMIC_STATE_SCISSOR，则视口和裁剪矩形的状态由VkPipelineViewportStateCreateInfo :: pViewports指定。因此，如果您在切换渲染管线后需要更改视口或裁剪矩形，则需要重新调用vkCmdSetViewport和vkCmdSetScissor。